...
import type { Category, Language, MenuItem, MenuViewStats, RestaurantMenu, Variant } from '../lib/types'
...
export function DashboardPage() {
  ...
  async function importCsv(event: React.FormEvent) {
    ...
    try {
      ...
      if (!rows.length) throw new Error(dashboardText(dashboardLanguage, 'Add at least one CSV row with name_en, name_ar and price_lbp columns.'))
      let nextMenu = menu
  ...
  return (
    <main className="dashboard-layout" dir={dashboardLanguage === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {itemModal && <div className="modal-backdrop"><form className="modal-card modal-large" onSubmit={saveItem}><button type="button" className="modal-close" onClick={() => { setItemModal(false); setEditingItem(null) }}><X /></button><span className="eyebrow"><span /> {editingItem ? t('Edit item') : t('New item')}</span><h2>{editingItem ? t('Edit menu item') : t('Add a menu item')}</h2><div className="form-grid"><label>{t('Category')}<select required value={itemDraft.category_id} onChange={(e) => setItemDraft({ ...itemDraft, category_id: e.target.value })}><option value="">{t('Choose category')}</option>{menu.categories.map((category) => <option value={category.id} key={category.id}>{category.name_en}</option>)}</select></label><label className="image-upload">{t('Item photo')}<span><ImagePlus /> {itemDraft.image_file?.name || (itemDraft.image_url ? t('Replace current photo') : t('Choose an image'))}</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => setItemDraft({ ...itemDraft, image_file: e.target.files?.[0] ?? null })} /></label><label>{t('English name')}<input required value={itemDraft.name_en} onChange={(e) => setItemDraft({ ...itemDraft, name_en: e.target.value })} placeholder="Margherita" /></label><label dir="rtl">{t('Arabic name')}<input required value={itemDraft.name_ar} onChange={(e) => setItemDraft({ ...itemDraft, name_ar: e.target.value })} placeholder="مارغريتا" /></label><label>{t('English description')}<textarea value={itemDraft.description_en} onChange={(e) => setItemDraft({ ...itemDraft, description_en: e.target.value })} placeholder="Tomato, mozzarella and basil" /></label><label dir="rtl">{t('Arabic description')}<textarea value={itemDraft.description_ar} onChange={(e) => setItemDraft({ ...itemDraft, description_ar: e.target.value })} placeholder="طماطم، موزاريلا وريحان" /></label></div><div className="price-section"><label>{t('Base price (LBP)')}<input required={itemDraft.variants.length === 0} min="0" type="number" value={itemDraft.price_lbp} onChange={(e) => setItemDraft({ ...itemDraft, price_lbp: e.target.value })} placeholder="350000" /></label><div className="variant-title"><div><b>{t('Size options')}</b><small>{t('Optional — add sizes when prices differ.')}</small></div><button type="button" onClick={addVariant}><Plus /> {t('Add size')}</button></div>{itemDraft.variants.map((variant, index) => <div className="variant-row" key={variant.id}><input required placeholder="S" value={variant.name_en} onChange={(e) => updateVariant(index, 'name_en', e.target.value)} /><input placeholder="ص" dir="rtl" value={variant.name_ar} onChange={(e) => updateVariant(index, 'name_ar', e.target.value)} /><input required min="1" type="number" placeholder="Price LBP" value={variant.price_lbp || ''} onChange={(e) => updateVariant(index, 'price_lbp', Number(e.target.value))} /><button type="button" onClick={() => setItemDraft((current) => ({ ...current, variants: current.variants.filter((_, position) => position !== index) }))}><X /></button></div>)}</div><button className="button button-primary full" disabled={saving || !menu.categories.length}>{saving ? t('Saving…') : editingItem ? t('Save menu item') : t('Add menu item')}</button></form></div>}

      {importModal && <div className="modal-backdrop"><form className="modal-card" onSubmit={importCsv}><button type="button" className="modal-close" onClick={() => { setImportModal(false); setImportStatus(''); setImportFile(null) }}><X /></button><span className="eyebrow"><span /> {t('Bulk import')}</span><h2>{t('Import menu items')}</h2><p>{t('Upload a CSV exported from Excel or Google Sheets. Columns: category_en, category_ar, name_en, name_ar, description_en, description_ar, price_lbp, available.')}</p><label className="file-drop"><Upload /><b>{importFile?.name || t('Choose CSV file')}</b><small>{t('One item per row')}</small><input type="file" accept=".csv,text/csv" onChange={(e) => setImportFile(e.target.files?.[0] ?? null)} /></label>{importStatus && <p className="notice notice-success">{importStatus}</p>}<button className="button button-primary full" disabled={saving || !importFile}>{saving ? t('Importing…') : t('Import items')}</button></form></div>}
