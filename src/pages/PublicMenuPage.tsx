import { ChevronRight, Clock3, Instagram, MapPin, MessageCircle, Phone, Search, Utensils } from 'lucide-react'
    ...
    <main className="public-menu" dir={rtl ? 'rtl' : 'ltr'} style={{ '--restaurant-color': restaurant.primary_color } as React.CSSProperties}>
      ...
      <footer className="menu-footer"><div className="footer-shell">
        ...
        <div className="footer-contact-grid">
          {restaurant.phone && <a className="footer-contact-item" href={`tel:${restaurant.phone}`}><span className="footer-contact-icon"><Phone /></span><span className="footer-contact-copy"><small>{language === 'ar' ? 'اتصل بنا' : 'Call us'}</small><strong>{restaurant.phone}</strong></span></a>}
          ...
          {restaurant.opening_hours && <span className="footer-contact-item"><span className="footer-contact-icon"><Clock3 /></span><span className="footer-contact-copy"><small>{language === 'ar' ? 'ساعات العمل' : 'Opening hours'}</small><strong>{restaurant.opening_hours}</strong></span></span>}
        </div>
