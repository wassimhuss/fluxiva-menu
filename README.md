...
The menu editor supports item/category editing and ordering, item photos, sold-out labels, quick theme presets, WhatsApp and Google Maps links, opening hours, and a bilingual preview. CSV imports use columns such as `category_en`, `category_ar`, `name_en`, `name_ar`, `description_en`, `description_ar`, `price_lbp`, and `available`.
...
2. Run `supabase/migrations/001_fluxiva_menu.sql` and `supabase/migrations/002_restaurant_details.sql` in its SQL editor.
